...
report = []
for phrase, expected_sha256, expected_md5 in phrases:
    calculated_sha256, calculated_md5 = calculate_hashes(phrase)
    sha256_status = "correto" if calculated_sha256 == expected_sha256 else "incorreto"
    md5_status = "correto" if calculated_md5 == expected_md5 else "incorreto"
    report.append((phrase, sha256_status, md5_status))

# Exibição do relatório
for phrase, sha256_status, md5_status in report:
    print(f"Frase: {phrase}")
    print(f"SHA256: {sha256_status}")
    print(f"MD5: {md5_status}")
    print("-" * 50)
